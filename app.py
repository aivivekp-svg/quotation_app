...
_ss_set("client_name", ""); _ss_set("client_type", ""); _ss_set("quote_no", "")
_ss_set("discount_pct", 0); _ss_set("letterhead", False)
_ss_set("client_addr", ""); _ss_set("client_email", ""); _ss_set("client_phone", "")
_ss_set("proposal_start", ""); _ss_set("sig_bytes", None)
...
def money_inr(n: float) -> str:
    ...
    neg = n < 0; n = abs(int(round(n))); s = str(n)
    if len(s) <= 3: res = s
    else:
        res = s[-3:]; s = s[:-3]
        while len(s) > 2:
...
def make_pdf(client_name: str, client_type: str, quote_no: str,
    ...
    def on_page(canv, doc_):
        pw, ph = A4
        canv.saveState()
        canv.setFillColor(colors.HexColor("#F7F9FC")); canv.rect(0,0,pw,ph,stroke=0,fill=1)
        x,y = card_inset, card_inset; w = pw-2*card_inset; h = ph-2*card_inset
        canv.setFillColor(colors.white); canv.setStrokeColor(colors.HexColor("#E5E7EB"))
        canv.setLineWidth(1); canv.rect(x,y,w,h,stroke=1,fill=1)
        if letterhead:
            try:
                for n in ("logo.png","logo.jpg","logo.jpeg"):
                    if os.path.exists(n):
                        ir = ImageReader(n); ow, oh = ir.getSize()
                        target_w = pw - 80*mm; r = target_w/ow; target_h = oh*r
                        cx = (pw-target_w)/2; cy = (ph-target_h)/2
    ...
    def on_page_end(canv, doc_):
        ...
        y_line = 20*mm; canv.setLineWidth(0.7)
    ...
    if logo_path and not letterhead:
        ...
        max_w, max_h = 26*mm, 26*mm; r = min(max_w/ow, max_h/oh)
        story.append(Image(logo_path, width=ow*r, height=oh*r)); story.append(Spacer(1,4))

    story.append(Paragraph("<b>V. Purohit & Associates</b>", styles["Title"]))
    story.append(Paragraph("<b>Annual Fees Proposal</b>", styles["h2"])); story.append(Spacer(1,6))
    ...
    if email and email.strip(): left_cells.append(Paragraph(f"<b>Email:</b> {email.strip()}", styles["Normal"]))
    if phone and phone.strip(): left_cells.append(Paragraph(f"<b>Phone:</b> {phone.strip()}", styles["Normal"]))
    ...
    story.append(meta_table); story.append(Spacer(1,8))
    ...
    table.setStyle(TableStyle([
        ("FONTSIZE",(0,0),(-1,0),10), ("BACKGROUND",(0,0),(-1,0),brand_blue), ("TEXTCOLOR",(0,0),(-1,0),colors.white),
        ("VALIGN",(0,0),(-1,0),"MIDDLE"), ("TOPPADDING",(0,0),(-1,0),6), ("BOTTOMPADDING",(0,0),(-1,0),6),
        ("BOX",(0,0),(-1,0),0.9,brand_blue), ("INNERGRID",(0,0),(-1,0),0.9,brand_blue),
        ("FONTSIZE",(0,1),(-1,-1),10), ("TOPPADDING",(0,1),(-1,-1),4), ("BOTTOMPADDING",(0,1),(-1,-1),4),
        ("ALIGN",(2,1),(2,-1),"RIGHT"),
        ("INNERGRID",(0,1),(-1,-1),0.3,colors.HexColor("#d9d9d9")), ("BOX",(0,0),(-1,-1),1.0,colors.black),
    ]))
    ...
    if not df_event.empty:
        ...
        ev.setStyle(TableStyle([
            ("FONTSIZE",(0,0),(-1,0),10), ("BACKGROUND",(0,0),(-1,0),brand_blue),
            ("TEXTCOLOR",(0,0),(-1,0),colors.white), ("VALIGN",(0,0),(-1,0),"MIDDLE"),
        ...
        story.append(ev); story.append(Spacer(1,10))
...
def export_proposal_excel(df_main, df_event, client_name, client_type, quote_no,
    ...
    headers = ["Service","Details","Annual Fees (Rs.)"]; ws.append(headers)
    ...
    for svc, grp in df_main.groupby("Service", sort=True):
        ...
        for _, r in g.iterrows():
            ...
            ws.append([svc if first else "", r.get("Details",""), amt]); first = False

    for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=3):
        row[2].number_format = '#,##,##0'
        for cell in row: cell.border = border_all

    ws.column_dimensions["A"].width = 28; ws.column_dimensions["B"].width = 52; ws.column_dimensions["C"].width = 18
    ...
    for i, (lbl, amt) in enumerate(items, start=start+1):
        ...
        c = ws.cell(row=i, column=3, value=int(round(amt))); c.number_format = '#,##,##0'; c.alignment = right
...
def build_status(df_app, df_fees):
    ...
    status = counts.merge(miss, on="ClientType"); status["ClientType"] = status["ClientType"].str.title()
    return status

# --------- UI ---------
st.set_page_config(page_title=APP_TITLE, page_icon="📄", layout="centered")

# Force light look even if browser/system is dark
st.markdown(f"""
<style>
:root {{ color-scheme: only light; }}
html, body, .stApp, .block-container {{ background: #FFFFFF !important; color: #111111 !important; }}
...
with st.sidebar:
    ...
    st.divider(); st.subheader("Options")
    ...
    with st.expander("Data status", expanded=False):
        ...
        st.write(f"Source: **{source}**"); st.write(f"Service definitions: **{service_defs}**")
...
st.title("Quotation Generator"); st.subheader("V. Purohit & Associates")
...
with st.form("quote_form", clear_on_submit=False):
    ...
    with colX: email = st.text_input("", st.session_state.get("client_email",""), placeholder="Email")
    with colY: phone = st.text_input("", st.session_state.get("client_phone",""), placeholder="Phone")
...
if st.session_state["editor_active"] and (
    ...
    st.success("Edit fees below. Event-based charges are listed separately and not included in totals.")

    # MAIN editor – Include, MoveToEvent, fees (text with Indian commas)
    if not st.session_state["quote_df"].empty:
            ...
            edited = st.data_editor(
                ...
                column_config={
                    ...
                    "Annual Fees (Rs.)": st.column_config.TextColumn(
                        "Annual Fees (Rs.)",
                        help="Use Indian format (e.g., 1,00,000). Only digits and commas allowed.",
                        validate="^\\s*[\\d,]*\\s*$",
                    ),
                },
        ...
        if apply_edits or apply_and_move:
            ...
            if apply_and_move:
                ...
                if not move_rows.empty:
                    ...
                    if ev_now.empty: ev_now = pd.DataFrame(columns=["Service","Details","Annual Fees (Rs.)","MoveToMain"])
        ...
        for col in ["Include","MoveToEvent"]:
            if col in filtered.columns: filtered.drop(columns=[col], inplace=True)

    else:
        filtered = pd.DataFrame(columns=["Service","Details","Annual Fees (Rs.)"])

    # EVENT editor – fees text + MoveToMain
    ...
    if not event_df.empty:
        ...
        if "MoveToMain" not in event_df.columns: event_df["MoveToMain"] = False
        ...
        if ev_apply:
            ...
            if not to_main.empty:
                ...
                add_main["Include"] = True; add_main["MoveToEvent"] = False
            ...
            if "MoveToMain" in keep_ev.columns: keep_ev["MoveToMain"] = False
            st.session_state["event_df"] = keep_ev

    # Totals (UI) — parse text to numbers
    calc_df = filtered.copy()
    calc_df["Annual Fees (Rs.)"] = calc_df["Annual Fees (Rs.)"].apply(parse_inr).fillna(0.0)
    subtotal, discount_amt, taxable, gst_amt, grand = compute_totals(calc_df, st.session_state["discount_pct"])
